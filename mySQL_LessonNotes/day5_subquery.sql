...
/* -----------------------------------------------------------------------------
  ORNEK3: Ankara’da calisani olan markalarin marka id'lerini,marka_isim ve calisan
...
/* -----------------------------------------------------------------------------
 ...
 select marka_isim, calisan_sayisi, (select round(AVG(maas)) from calisanlar -- round ==> yuvarladi
										where markalar.marka_isim=calisanlar.isyeri ) as maas_Ort
from markalar;
...
/* ----------------------------------------------------------------------------
 ...
 select marka_id, marka_isim, (select COUNT(sehir) from calisanlar 
										where markalar.marka_isim=calisanlar.isyeri ) sehir_sayisi
 from markalar;
