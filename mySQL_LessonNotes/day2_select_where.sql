...
/*-----------------------------------------------------------------------------
 ...
 select * from ogrenciler -- * girince tum bilgileri getirdi
 where sinav_notu > 80;
 ...
 ORNEK2: Adresi Ankara olan öğrencilerin isim ve adres bilgilerini listele
  ...
  select isim , adres from ogrenciler  -- * => yerine istedigimiz verileri girdik 
  where adres='Ankara';
  ...
  select * from ogrenciler where id=124;
   ...
   /*======================= SELECT - BETWEEN ==================================*/ 
    ...
    -- yazdiginiz 2 sinir da araliğa dahildir
...
/*-----------------------------------------------------------------------------
  ...
  -- 2.Yol
  select * from personel where id >= '10002' AND id <= '10005'; -- matematiksel cozumu
  ...
  select * from personel where isim between 'Mehmet Yılmaz' AND 'Veli Han '; -- 1. Yol
  select * from personel where isim >= 'Mehmet Yılmaz' AND isim <='Veli Han '; -- 2.Yol
...
-----------------------------------------------------------------------------*/   
 ...
 -- dogru yazim
 select * from personel
 where maas=7000 or isim='meryem'; -- in kullanilamaz 
 -- yanlis yazim
 select * from personel
...
/* -----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/ 
 -- 1. Yol -> Burada kucuk buyuk harfe dikkat et yapamayiz
 select * from kelimeler
...
/* -----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/ 
select * from kelimeler
where REGEXP_LIKE(kelime, 'h[ai]t' ); -- [] -> 2 gorevi var bir kelime oldugunu belirtir ve 
									  --       veya anlamida vardir		
...
-----------------------------------------------------------------------------*/ 
 select * from kelimeler
where REGEXP_LIKE(kelime, '^[as]' ); -- ^a|^s
/* -----------------------------------------------------------------------------
  ORNEK30: içerisinde en az 2 adet oo barıdıran kelimelerin tüm bilgilerini 
  listeleyiniz.
-----------------------------------------------------------------------------*/ 
select * from kelimeler
where REGEXP_LIKE(kelime,'[o]{2}'); -- [o][o] da en az 2 ve ustunu getirir
-- [x]{a} --> en az a adet barindaran x harfi olan kelimeleri getiriyor 
