...
==============================================================================*/
 ...
 where urun_isim in(select urun_isim from mart as m
...
update insanlar
set isim='NO NAME' -- no name olarak degistir
where isim is null; -- ismi null olanlari

 select * from insanlar;
 
 -- tabloyu eski haline dondurduk
 update insanlar
set isim=null 
where isim='NO NAME';
...
-----------------------------------------------------------------------------*/    
update insanlar
set  isim=COALESCE(İSİM, 'Henuz isim girilmedi'),
...
/* ----------------------------------------------------------------------------
  ORNEK2: kisiler tablosunu maas'a göre ters den sıralayarak sorgulayınız.
 -----------------------------------------------------------------------------*/
 SELECT * FROM kisiler
 order by maas desc;
...
select * from kisiler
where soyisim='Bulut'
order by 5 ; -- maas'ın tablo daki sutun sayisi da yazilabilir
...
-- 1.Yol
select*from kisiler limit 10,2; -- ilk siradaki veriyi almaz sonraki 2 veriyi getirir
-- 2.Yol
select * from kisiler where id > 10 limit 2; -- id si 10'dan buyuk 2 tane getirir
