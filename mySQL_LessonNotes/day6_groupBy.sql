...
-----------------------------------------------------------------------------*/ 
 ...
 ----------------------------------------------------------------------------------*/  
    ...
    from manav
	group by isim, urun_adi
    having sum(urun_miktari) >= 3 -- => where yerine group by ve sum gibi aggregate metodlar kullandigimizda having kullaniriz
    order by sum(urun_miktari) desc;
 
 -- aggregate fonksiyonlari ile ilgili bir kosul koymak 
 -- icin group by ' dan sonra having cumlecigi kullanilir
  ...
  /* ----------------------------------------------------------------------------
		...
		sorguyu yazınız. 
        NOT: Sorgu, sadece MAKS urun_miktari MIN urun_miktarına 
...
--------------------------------------------------------------------------*/	
	select count(distinct urun_adi ) -- distinct den sonra paranyeze gerek yok
    from manav;
